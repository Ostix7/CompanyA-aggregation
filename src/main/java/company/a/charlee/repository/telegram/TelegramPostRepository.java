...
//    List<Object[]> findPostsWithMostDiverseEmojis(Pageable pageable);
    ...
    @Query(nativeQuery = true, value =
            ...
            "WHERE t1.segment = :segment AND (EXTRACT(EPOCH FROM t1.post_date) BETWEEN :minDate AND :maxDate) AND t1.is_processed = TRUE\n" +
            "GROUP BY aggregated_date\n" +
            "ORDER BY aggregated_date\n")
    List<Object[]> getAverageSentimentValuesForSegmentPostsByDates(
    ...
    @Query(nativeQuery = true, value =
            "SELECT to_timestamp(:minDate + (DIV(EXTRACT(EPOCH FROM DATE_TRUNC(:timeFrame, t1.post_date)) - :minDate, :interval) * :interval)) AS aggregated_date, SUM(t1.view_count)\n" +
            "FROM telegram_post t1 JOIN telegram_channel t2 ON t1.channel_id = t2.channel_id\n" +
            "WHERE t2.channel_title = :telegramChannelTitle AND (EXTRACT(EPOCH FROM t1.post_date) BETWEEN :minDate AND :maxDate)\n" +
            "GROUP BY aggregated_date\n" +
            "ORDER BY aggregated_date, SUM(t1.view_count) DESC\n")
    List<Object[]> getTotalPostsViewsByDates(
            @Param("telegramChannelTitle") String telegramChannelTitle,
            @Param("timeFrame") String timeFrame,
            @Param("interval") long interval,
            @Param("minDate") long minDate,
    ...
    @Query(nativeQuery = true, value =
            "SELECT to_timestamp(:minDate + (DIV(EXTRACT(EPOCH FROM DATE_TRUNC(:timeFrame, t1.post_date)) - :minDate, :interval) * :interval)) AS aggregated_date, SUM(t1.view_count)\n" +
                    "FROM telegram_post t1\n" +
                    "WHERE t1.segment = :segment AND (EXTRACT(EPOCH FROM t1.post_date) BETWEEN :minDate AND :maxDate)\n" +
                    "GROUP BY aggregated_date\n" +
                    "ORDER BY aggregated_date, SUM(t1.view_count) DESC\n")
    List<Object[]> getTotalPostsViewsByDatesForSegment(
            @Param("segment") String segment,
            @Param("timeFrame") String timeFrame,
            @Param("interval") long interval,
            @Param("minDate") long minDate,
    ...
    @Query(nativeQuery = true, value =
            ...
            "        JOIN telegram_post t2 ON t1.telegram_post_id = t2.id\n" +
            "        JOIN telegram_channel t3 ON t2.channel_id = t3.channel_id\n" +
            "        WHERE EXTRACT(EPOCH FROM t2.post_date) BETWEEN :minDate AND :maxDate AND t2.segment = :segment\n" +
            "    ) AS base_data\n" +
            "    GROUP BY aggregatedDate, word\n" +
            ") AS ranked_words\n" +
            "WHERE rn <= :topicsLimitByDate\n" +
            "ORDER BY aggregatedDate, occurrences DESC;\n")
    List<Object[]> getPostTopicModelingOccurrencesByDatesForSegment(
            @Param("segment") String segment,
            @Param("topicsLimitByDate") Integer topicsLimitByDate,
            @Param("timeFrame") String timeFrame,
            @Param("interval") long interval,
    ...
    @Query(nativeQuery = true, value =
            "SELECT aggregatedDate AS aggregatedDate, word AS topic, occurrences AS occurrences " +
                    ...
                    "WHERE rn <= :topicsLimitByDate\n" +
                    "ORDER BY aggregatedDate, occurrences DESC;\n")
    List<Object[]> getPostPhraseTopicModelingOccurrencesByDatesForSegment(
            @Param("segment") String segment,
            @Param("topicsLimitByDate") Integer topicsLimitByDate,
            @Param("timeFrame") String timeFrame,
            @Param("interval") long interval,
    ...
    @Query(nativeQuery = true, value =
            "SELECT to_timestamp(:minDate + (DIV(EXTRACT(EPOCH FROM DATE_TRUNC(:timeFrame, t2.post_date)) - :minDate, :interval) * :interval)) AS aggregated_date, COUNT(t1.comment_id)\n" +
            "FROM telegram_comment t1 JOIN telegram_post t2 ON t1.post_id = t2.id JOIN telegram_channel t3 ON t2.channel_id = t3.channel_id \n" +
            "WHERE t2.segment = :segment AND (EXTRACT(EPOCH FROM t2.post_date) BETWEEN :minDate AND :maxDate) \n" +
            "GROUP BY aggregated_date\n" +
            "ORDER BY aggregated_date\n")
    List<Object[]> getCommentsEngagementForPostsByDatesForSegment(
            @Param("segment") String segment,
            @Param("timeFrame") String timeFrame,
            @Param("interval") long interval,
            @Param("minDate") long minDate,
    ...
    @Query(nativeQuery = true, value =
            ...
            "GROUP BY aggregated_date\n" +
            "ORDER BY aggregated_date\n")
    List<Object[]> getReactionsEngagementForPostsByDatesForSegment(
            @Param("segment") String segment,
            @Param("timeFrame") String timeFrame,
            @Param("interval") long interval,
            @Param("minDate") long minDate,
